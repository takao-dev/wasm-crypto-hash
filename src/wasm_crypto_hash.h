/* Renamed header for wasm-crypto-hash (wasm-crypto-hash project)
   Original: hash-wasm.h (kept for compatibility historically)
*/
#include <stdint.h>
#include <stdalign.h>
...
static __inline__ void* memcpy2(void* dst, const void* src, uint32_t cnt) {
  ...
  const uint8_t *source = (const uint8_t*)source64;
  while (cnt) {
    *(destination++)= *(source++);
    --cnt;
  }
